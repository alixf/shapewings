...
class BaseTexturedMesh extends hxsl.Shader {
	static var SRC = {
		@global var camera : {
			var view : Mat4;
			var proj : Mat4;
		...
		function __init__() {
			...
			uv = input.uv;
			diff = dot(transformedNormal, normalize(vec3(0,2,-1)));
		}
		...
		function fragment() {
			output.color = pixelColor * textureColor * diff;
			output.color += pixelColor * textureColor * ambient;
